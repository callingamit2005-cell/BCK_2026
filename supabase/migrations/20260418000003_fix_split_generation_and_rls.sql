...
-- Migration: fix_split_generation_and_rls
-- Description: Fixes split generation failure by relaxing RLS and perfecting user_id mapping.

-- 1. Relax RLS on expense_splits to allow group-wide split management
...
WITH CHECK (
  EXISTS (
    ...
    WHERE group_members.group_id = expense_splits.group_id
    AND group_members.user_id = auth.uid()
  )
);

DROP POLICY IF EXISTS "Users can view expense splits" ON public.expense_splits;
DROP POLICY IF EXISTS "Users see group splits" ON public.expense_splits;
...
-- 2. Refined Atomic RPC with perfected user_id mapping
...
BEGIN
    ...
    ELSIF p_split_type = 'unequal' AND p_custom_splits IS NOT NULL THEN
        ...
        IF ABS(v_total_split_sum - p_amount) > 0.1 THEN
