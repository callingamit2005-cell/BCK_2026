-- 🛠️ HARDENED ATOMIC INVITE SYSTEM RPC: join_group_via_token (V2)
-- Mission: Resolve parameter ambiguity, execution sequence integrity, and API flexibility.
-- Status: Production Safe | Atomic | Hardened.

CREATE OR REPLACE FUNCTION public.join_group_via_token(
  invite_token UUID,
  user_id UUID DEFAULT auth.uid() -- ADDED: Default to session auth for flexibility
)
RETURNS JSON
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- 5. SECURE MEMBER INSERTION
  -- Uses ON CONFLICT to maintain idempotency.
  ...
  -- 6. POST-INSERT STATUS DETECTION
