-- 🛠️ SUPABASE RPC FIX: insert_group_expense_with_split
-- Objective: Ensure member_id persistence in expense_splits and support ghost members.
...
BEGIN
    ...
    IF p_splits IS NOT NULL AND jsonb_array_length(p_splits) > 0 THEN
        ...
        -- Check for duplicate member_ids in the payload (Crucial for ghost support)
        ...
        LOOP
            ...
            -- Fallback to lookup if member_id is missing but user_id is present
...
-- 🛡️ PRODUCTION BACKFILL: Restore identity for existing rows
-- Objective: Repair rows that were inserted without member_id (but have user_id)
