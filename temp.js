...
var init_definitions = __esm({
  "node_modules/@capacitor-community/sqlite/dist/esm/definitions.js"() {
    SQLiteConnection = class {
      ...
      async createConnection(database, encrypted, mode, version5, readonly) {
        try {
          if (database.endsWith(".db"))
            database = database.slice(0, -3);
      ...
      async createNCConnection(databasePath, version5) {
        try {
    ...
    SQLiteDBConnection = class {
      ...
      async getVersion() {
        try {
          const version5 = await this.sqlite.getVersion({
            database: this.dbName,
            readonly: this.readonly
          });
          return Promise.resolve(version5);
        } catch (err) {
          return Promise.reject(err);
        }
      }
...
__export(sqlite_exports, {
  ...
  getLocalTransactions: () => getLocalTransactions,
  initSQLite: () => initSQLite,
  saveLocalTransaction: () => saveLocalTransaction
});
var sqlite, db, initPromise, initSQLite, ensureDbReady, getDB2, cachedDeviceId, getDeviceId, generateIdempotencyKey, emitSyncQueueUpdated, enqueueSync, saveLocalTransaction, getAggregateStats, getCategoryTotals, getLocalTransactions;
  ...
  "src/integrations/sqlite.ts"() {
    ...
    initSQLite = async () => {
      ...
      CREATE TABLE IF NOT EXISTS transactions (
        ...
        category TEXT, payment_mode TEXT, description TEXT, date TEXT NOT NULL, sms_hash TEXT UNIQUE,
        entry_source TEXT DEFAULT 'sms',
        sync_status TEXT DEFAULT 'pending', is_deleted INTEGER DEFAULT 0,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
      );

      -- LEGACY_EXPENSES_TABLE: Added for backward compatibility and ledger stability
      CREATE TABLE IF NOT EXISTS expenses (
        id TEXT PRIMARY KEY, user_id TEXT NOT NULL, amount INTEGER NOT NULL,
        category TEXT NOT NULL, note TEXT, payment_mode TEXT, expense_date TEXT,
        group_id TEXT, created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        is_deleted INTEGER DEFAULT 0, sync_status TEXT DEFAULT 'completed'
      );
      ...
      CREATE TABLE IF NOT EXISTS emis (
        ...
        sync_status TEXT DEFAULT 'pending', is_deleted INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        updated_at DATETIME DEFAULT CURRENT_TIMESTAMP, is_latest INTEGER DEFAULT 1, version INTEGER,
        emi_name TEXT -- legacy compatibility
      );
    ...
    `;
          ...
          try {
            await db.run("ALTER TABLE emis ADD COLUMN emi_day INTEGER");
          } catch (e) {
          }
          try {
            await db.run("ALTER TABLE emis ADD COLUMN name TEXT");
          } catch (e) {
          }
          ...
          try {
            for (const col of requiredColumns) {
              ...
              if (!txCols.includes(col.name)) {
                console.log(`\u{1F6E0}\uFE0F [SQLITE_MIGRATION] Adding missing column: ${col.name}`);
                await db.run(`ALTER TABLE transactions ADD COLUMN ${col.name} ${col.type}`);
                const verifyInfo = await db.query("PRAGMA table_info(transactions)");
                const verifyCols = (verifyInfo.values || []).map((c) => c.name);
                if (!verifyCols.includes(col.name)) {
                  throw new Error(`CRITICAL: Failed to add column ${col.name} to transactions table.`);
                }
              }
            }
            const indexInfo = await db.query("PRAGMA index_list(transactions)");
            const indices = (indexInfo.values || []).map((i) => i.name);
            if (!indices.includes("idx_transactions_sms_hash")) {
              await db.run("CREATE UNIQUE INDEX IF NOT EXISTS idx_transactions_sms_hash ON transactions(sms_hash)");
            }
            if (!indices.includes("idx_transactions_user_date")) {
              await db.run("CREATE INDEX IF NOT EXISTS idx_transactions_user_date ON transactions(user_id, date)");
            }
          } catch (e) {
            console.error("\u274C [SQLITE_MIGRATION_FAIL] Transactions schema migration failed:", e);
            throw e;
          }
          try {
            ...
            await db.run(`UPDATE transactions SET entry_source = 'income' WHERE (entry_source IS NULL OR entry_source = 'sms') AND (sms_hash LIKE 'inc:%' OR type = 'income')`);
          } catch (e) {
            console.warn("Legacy source recovery failed:", e);
          }
          try {
            await db.run("UPDATE emis SET name = emi_name WHERE name IS NULL AND emi_name IS NOT NULL");
          } catch (e) {
          }
          let deviceId = localStorage.getItem("bk_device_id");
          try {
            const res = await db.query(`SELECT value FROM app_settings WHERE key = 'device_id'`);
            if (res.values && res.values.length > 0) {
              const sqlDeviceId = res.values[0].value;
              if (!deviceId) {
                deviceId = sqlDeviceId;
              ...
              await db.run(`INSERT INTO app_settings (key, value) VALUES ('device_id', ?)`, [deviceId]);
            }
            cachedDeviceId = deviceId;
          } catch (e) {
            console.warn("Failed to sync deviceId with SQLite:", e);
            if (!deviceId) {
              deviceId = `dev_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
              localStorage.setItem("bk_device_id", deviceId);
            }
    ...
    ensureDbReady = async () => {
      ...
      const isOpen = db ? (await db.isDBOpen()).result : false;
      if (!db || !isOpen) {
        console.warn("\u26A0\uFE0F [SQLITE_RECOVERY] Connection lost or not opened. Triggering (re)init...");
        initPromise = null;
        await initSQLite();
      }
      return db;
    ...
    generateIdempotencyKey = async (payload) => {
      try {
        const payloadWithDevice = { ...payload, __deviceId: getDeviceId() };
        ...
        const deterministicString = JSON.stringify(sortObject(payloadWithDevice));
        ...
        const str = JSON.stringify({ ...payload, __deviceId: getDeviceId() });
        let hash = 0;
        for (let i = 0; i < str.length; i++) {
          hash = (hash << 5) - hash + str.charCodeAt(i);
          hash |= 0;
        }
        return `fb:${Math.abs(hash)}_${Date.now()}`;
      }
    };
    ...
    enqueueSync = async (tableName, operation, payload, silent = false) => {
      const currentDb = await ensureDbReady();
      if (!currentDb) return false;
      try {
        const idempotencyKey = payload.idempotency_key || payload.p_idempotency_key || payload.sms_hash || payload.id || payload.p_id || await generateIdempotencyKey(payload);
        const syncId = `sq_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
        const query = `
    ...
    `;
        await currentDb.run(query, [syncId, tableName, operation, JSON.stringify(payload), idempotencyKey]);
        if (!silent) {
          emitSyncQueueUpdated();
        }
        return true;
      } catch (err) {
        console.error("Failed to enqueue sync:", err);
        return false;
      }
    };
    saveLocalTransaction = async (tx, silent = false) => {
      const currentDb = await ensureDbReady();
      if (!currentDb) {
        if (import.meta.env.DEV) console.warn("[FORENSIC_DB_MISSING] saveLocalTransaction called before DB init.");
        return null;
      }
      try {
        const transactionId = tx.id || crypto.randomUUID();
        const finalTx = { ...tx, id: transactionId };
        const query = `
      INSERT INTO transactions (
        id, user_id, amount, type, category, payment_mode, 
        description, date, sms_hash, entry_source, sync_status, is_deleted
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(sms_hash) DO UPDATE SET
        ...
        date = excluded.date,
        entry_source = COALESCE(excluded.entry_source, transactions.entry_source),
        sync_status = excluded.sync_status,
        is_deleted = excluded.is_deleted,
        updated_at = CURRENT_TIMESTAMP
      WHERE transactions.is_deleted = 0 OR excluded.is_deleted = 1 OR excluded.is_deleted = 0;
    `;
        const params = [
          finalTx.id,
          finalTx.user_id,
          ...
          finalTx.type,
          finalTx.category,
          finalTx.payment_mode,
          finalTx.description,
          finalTx.date,
          finalTx.sms_hash || finalTx.smsHash || null,
          finalTx.entry_source || finalTx.source || "sms",
          finalTx.sync_status || "pending",
          finalTx.is_deleted || 0
        ];
        const result = await currentDb.run(query, params);
        if (import.meta.env.DEV) {
          const rowsAffected = result.changes?.changes || 0;
          console.log(`[FORENSIC_INSERT] Table: transactions`);
          console.log(`[FORENSIC_INSERT] Rows Affected: ${rowsAffected}`);
          if (rowsAffected > 0) {
            console.log(`[FORENSIC_INSERT] First Inserted Row Sample:`, JSON.stringify(finalTx));
          }
        }
        if (!silent) {
          emitSyncQueueUpdated();
        ...
        console.error(`[FORENSIC_INSERT_CRASH] ID: ${tx.id}`, {
          message: err.message,
          code: err.code,
          payload: tx
        });
        throw new Error(`SQLite Insertion Deadlock: ${err.message}`);
      }
    };
    getAggregateStats = async (userId, monthYear) => {
      const currentDb = await ensureDbReady();
      ...
      try {
        ...
        const query = `
      SELECT 
        COALESCE(SUM(CASE WHEN type != 'income' AND (entry_source != 'income') THEN amount ELSE 0 END), 0) as total_spent,
        COALESCE(SUM(CASE WHEN (type != 'income' AND entry_source != 'income') ${monthlyQuery} THEN amount ELSE 0 END), 0) as monthly_spent,
        COALESCE(SUM(CASE WHEN (type = 'income' OR entry_source = 'income') ${monthlyQuery} THEN amount ELSE 0 END), 0) as monthly_income
      FROM transactions 
      WHERE user_id = ? AND COALESCE(is_deleted, 0) = 0
    `;
        const finalParams = monthYear ? [userId, monthYear, monthYear] : [userId];
        const res = await currentDb.query(query, finalParams);
        return res.values?.[0] || { total_spent: 0, monthly_spent: 0, monthly_income: 0 };
      } catch (err) {
        console.error("Aggregate stats failed:", err);
        return { total_spent: 0, monthly_spent: 0, monthly_income: 0 };
      }
    };
    getCategoryTotals = async (userId, monthYear) => {
      const currentDb = await ensureDbReady();
      if (!currentDb) return [];
      try {
        const query = `
      ...
      WHERE user_id = ? 
        ...
        AND (type != 'income' AND entry_source != 'income')
      GROUP BY category
      ORDER BY amount DESC
    `;
        const res = await currentDb.query(query, [userId, monthYear]);
        return res.values || [];
      } catch (err) {
        console.error("Category totals failed:", err);
        return [];
      }
    };
    getLocalTransactions = async (userId, limit = 500) => {
      if (!db) return [];
      try {
        const query = `SELECT * FROM transactions WHERE user_id = ? AND COALESCE(is_deleted, 0) = 0 ORDER BY date DESC LIMIT ?`;
        const res = await db.query(query, [userId, limit]);
        return res.values || [];
      } catch (err) {
        console.error("Fetch local transactions failed:", err);
        return [];
      }
    };
  }
  ...
  SUPABASE_PUBLISHABLE_KEY || "",
  {
    ...
    global: {
      ...
      fetch: async (...args) => {
        ...
        if (isRefresh && refreshBreaker.isDown && Date.now() - refreshBreaker.lastFailureTime < 6e4) {
        ...
        const executeFetch = async () => {
          const MAX_RETRIES = isManualAuth ? 1 : 3;
          let attempt = 0;
          while (attempt <= MAX_RETRIES) {
            const controller = new AbortController();
            const timeoutId = setTimeout(() => controller.abort("Transport Timeout"), 2e4);
            try {
              ...
              clearTimeout(timeoutId);
              attempt++;
              const isTransportError = err.name === "AbortError" || err.message?.includes("Failed to fetch") || err.message?.includes("QUIC_PROTOCOL_ERROR") || err.name === "TypeError";
              if (isTransportError) {
...
// src/features/transactions/ledger.ts
init_dist();
...
var isCurrentMonth = (date) => {
  const d = new Date(date);
  const now = /* @__PURE__ */ new Date();
  return d.getMonth() === now.getMonth() && d.getFullYear() === now.getFullYear();
};
var isLastMonth = (date, now = /* @__PURE__ */ new Date()) => {
  const d = new Date(date);
...
var normalizePaymentMode = (paymentMode, type = "expense") => {
  if (paymentMode && paymentMode.trim()) {
    return paymentMode.trim();
  }
  return type === "income" ? "Bank Credit" : "Smart Entry";
};
var toIsoString = (value) => {
  if (!value || !value.trim()) return (/* @__PURE__ */ new Date()).toISOString();
  let normalized = typeof value === "string" ? value.trim() : value;
  if (/^\d{4}-\d{2}-\d{2}$/.test(normalized)) {
    return (/* @__PURE__ */ new Date(`${normalized}T00:00:00`)).toISOString();
  }
...
var isWithinLedgerWindow = (dateLike, window2) => {
  if (!dateLike) return false;
  const date = safeDate(dateLike);
  if (!date || Number.isNaN(date.getTime())) return false;
  return date >= window2.start && date <= window2.end;
};
var derivePayeeName = (row) => {
  return row.merchantName || row.payee || row.sender || row.description || row.category || "SMS Transaction";
};
var isValidSmsTransaction = (row) => {
  if (!row) return false;
  const source = String(row.entry_source || row.source || "").toLowerCase();
  const hash = String(row.sms_hash || row.smsHash || "");
  if (hash && (source === "sms" || row.origin === "native-transaction")) {
    if (import.meta.env.DEV) {
      console.log(`[FORENSIC_FIREWALL] TRUST: Native SMS detected. Hash: '${hash.substring(0, 8)}'`);
    }
    return true;
  }
  const isManualSource = source && ["manual", "voice", "paste", "salary", "income", "legacy-manual"].includes(source);
  const isManualHash = hash.startsWith("man:") || hash.startsWith("voice:") || hash.startsWith("inc:");
  if (isManualSource || isManualHash) {
    if (import.meta.env.DEV) {
      console.log(`[FORENSIC_FIREWALL] BYPASS: Manual entry detected. Source: '${source}', Hash: '${hash.substring(0, 8)}'`);
    }
    return true;
  }
...
var toUnifiedTransactionEntry = (row) => {
  ...
  if (import.meta.env.DEV) {
    console.log(`[FORENSIC_MAPPER] Type: Cloud/SQLite, ID: ${row.id}, Amount: ${amount}, Deleted: ${isDeleted}`);
  }
  const rawSource = row.entry_source || row.source || "";
  const hash = smsHash || "";
  let finalSource = "manual";
  if (rawSource) {
    finalSource = normalizeSource(rawSource);
  } else if (hash.startsWith("man:") || hash.startsWith("inc:")) {
    finalSource = "manual";
  } else if (hash.startsWith("voice:")) {
    finalSource = "voice";
  } else if (hash) {
    finalSource = "sms";
  } else {
  ...
  return {
    ...
    paymentMode: row.payment_mode || row.paymentMode || normalizePaymentMode(null, type),
    date: toIsoString(row.date),
    note: row.description || row.note || payee,
    payee,
    direction: type === "income" ? "credit" : "debit",
    type,
    source: finalSource,
    origin: row.origin || "native-transaction",
    smsHash,
...
var toUnifiedNativeEntry = (row) => {
  const type = String(row.type || "").toLowerCase() === "credit" ? "income" : "expense";
  const payee = derivePayeeName(row);
  const amount = Number(row.amount || 0);
  const date = row.date || (row.timestamp ? new Date(Number(row.timestamp)).toISOString() : (/* @__PURE__ */ new Date()).toISOString());
  const smsHash = row.smsHash || row.sms_hash || null;
  const isDeleted = Boolean(row.is_deleted);
  if (import.meta.env.DEV) {
    console.log(`[FORENSIC_MAPPER] Type: NativeBridge, ID: ${row.id}, Amount: ${amount}, Deleted: ${isDeleted}`);
  }
  return {
    id: String(row.id),
    amount,
    ...
    smsHash,
    idempotencyKey: row.idempotencyKey || row.idempotency_key || smsHash || String(row.id),
    updatedAt: row.updatedAt || row.updated_at || date || null,
    isDeleted
  };
};
var toUnifiedSalaryEntry = (salaryAmount, date) => {
  const amount = Number(salaryAmount || 0);
  if (import.meta.env.DEV) {
    console.log(`[FORENSIC_MAPPER] Type: Salary, Amount: ${amount}`);
  }
  return {
    id: `salary:${date}`,
    amount,
...
var getEntryKey = (entry) => {
  return entry.idempotencyKey || entry.idempotency_key || entry.smsHash || entry.sms_hash || entry.id;
};
var mergeUnifiedLedgerEntries = (entries) => {
  ...
  const getTs = (e) => {
    const cacheKey = e.id || "missing-id";
    if (timeCache.has(cacheKey)) return timeCache.get(cacheKey);
    const raw = e.updatedAt || e.updated_at || e.created_at || e.date;
    const time = safeTime(raw);
    timeCache.set(cacheKey, time);
    return time;
  };
  for (const entry of entries || []) {
    if (!entry) continue;
    const key = getEntryKey(entry) || entry.id || `fallback_${Math.random()}`;
    if (entry.isDeleted) {
      console.log(`[FORENSIC_TOMBSTONE_ENFORCED] ID: ${entry.id}, Key: ${key}, Source: ${entry.origin} -> REJECTED`);
      deletedKeys.add(key);
      deduped.delete(key);
      continue;
    }
    if (deletedKeys.has(key)) {
      console.log(`[FORENSIC_RESURRECTION_PREVENTED] ID: ${entry.id}, Key: ${key}, Source: ${entry.origin} -> BLOCKED BY TOMBSTONE`);
      continue;
    }
    const existing = deduped.get(key);
    if (!existing) {
      if (import.meta.env.DEV) console.log(`[FORENSIC_MERGE] Accepting fresh entry [${key}] from ${entry.origin}`);
      deduped.set(key, entry);
      continue;
    }
    const existingTime = getTs(existing);
    const entryTime = getTs(entry);
    if (entryTime > existingTime) {
      if (import.meta.env.DEV) console.log(`[FORENSIC_MERGE] Overwriting [${key}]. ${entry.origin} (${entryTime}) > ${existing.origin} (${existingTime})`);
      deduped.set(key, entry);
    } else if (entryTime === existingTime) {
      if (entryPriority[entry.origin] > entryPriority[existing.origin]) {
        if (import.meta.env.DEV) console.log(`[FORENSIC_MERGE] Tie-break overwrite [${key}]. ${entry.origin} Priority > ${existing.origin}`);
        deduped.set(key, entry);
      } else {
        if (import.meta.env.DEV) console.log(`[FORENSIC_MERGE] Dedupe Rejection [${key}]. ${entry.origin} lost to ${existing.origin}`);
      }
    } else {
      if (import.meta.env.DEV) console.log(`[FORENSIC_MERGE] Dedupe Rejection [${key}]. ${entry.origin} is older than ${existing.origin}`);
    }
  }
  return Array.from(deduped.values()).filter((e) => !deletedKeys.has(getEntryKey(e) || e.id)).sort((a, b) => {
...
var toUnifiedExpenseEntry = (row) => {
  const payee = row.name || row.description || row.category || "Smart Entry";
  const rawDate = row.expense_date || row.created_at;
  const amount = normalizeToPaisa(row.amount, true);
  if (import.meta.env.DEV) {
    console.log(`[FORENSIC_MAPPER] Type: LegacyExpense, ID: ${row.id}, Amount: ${amount}`);
  }
  return {
    id: row.id,
    amount,
    category: row.category || "Others",
    paymentMode: row.payment_mode || normalizePaymentMode(null, "expense"),
    date: toIsoString(rawDate),
    note: row.note || row.description || payee,
    payee,
    direction: "debit",
    type: "expense",
...
var fetchUnifiedLedger = async (userId, window2) => {
  let localRows = [];
  let localExpRows = [];
    ...
    if (db2) {
      try {
        const [txRes, expRes] = await Promise.allSettled([
          db2.query(`SELECT * FROM transactions WHERE user_id = ?`, [userId]),
          db2.query(`SELECT * FROM expenses WHERE user_id = ?`, [userId])
        ]);
        if (txRes.status === "fulfilled") localRows = txRes.value.values || [];
        if (expRes.status === "fulfilled") localExpRows = expRes.value.values || [];
        console.log(`[FORENSIC_SELECT] Table: transactions, Count: ${localRows.length}`);
        console.log(`[FORENSIC_SELECT] Table: expenses, Count: ${localExpRows.length}`);
      } catch (err) {
        console.warn("\u26A0\uFE0F [Ledger] SQLite query failed (likely table missing):", err);
      }
    }
  }
  let cloudTxRows = [];
  let cloudExpRows = [];
  if (navigator.onLine) {
    if (import.meta.env.DEV) console.log(`[FORENSIC_SYNC] Network Online. Fetching cloud transactions to merge with ${localRows.length} local rows.`);
    try {
      const [txResult, expResult] = await Promise.all([
        supabase.from("transactions").select("*").eq("user_id", userId).eq("is_deleted", false).order("date", { ascending: false }).limit(500),
        supabase.from("expenses").select("*").eq("user_id", userId).order("expense_date", { ascending: false }).limit(500)
      ]);
      if (!txResult.error) cloudTxRows = txResult.data || [];
      if (!expResult.error) cloudExpRows = expResult.data || [];
      if (import.meta.env.DEV) console.log(`[FORENSIC_SYNC] Cloud fetch successful. Txs: ${cloudTxRows.length}, Exps: ${cloudExpRows.length}`);
    } catch (err) {
      console.warn("\u26A0\uFE0F [Ledger] Cloud fetch failed, falling back to local only", err);
    }
  } else {
    if (import.meta.env.DEV) console.log(`[FORENSIC_SYNC] Network Offline. Relying entirely on ${localRows.length} local rows.`);
  }
  console.log(`[FORENSIC_FILTER] Before filter: ${localRows.length} local rows fetched from SQLite.`);
  const localTxEntries = localRows.filter(isValidSmsTransaction).map(toUnifiedTransactionEntry).filter((entry) => isWithinLedgerWindow(entry.date, window2));
  console.log(`[FORENSIC_FILTER] After filter: ${localTxEntries.length} local rows remaining.`);
  const localLegacyEntries = localExpRows.map((row) => toUnifiedExpenseEntry(row)).filter((entry) => isWithinLedgerWindow(entry.date, window2));
  const cloudTxEntries = cloudTxRows.filter(isValidSmsTransaction).map(toUnifiedTransactionEntry).filter((entry) => isWithinLedgerWindow(entry.date, window2));
  const cloudExpEntries = cloudExpRows.map((row) => toUnifiedExpenseEntry(row)).filter((entry) => isWithinLedgerWindow(entry.date, window2));
...
// src/integrations/smsBridge.ts
init_dist();
var SmsBridge = registerPlugin("SmsBridge");
var addSmsListener = async (eventName, callback) => {
  if (Capacitor.getPlatform() !== "android") {
    return { remove: () => {
    } };
  }
...
var getNativeTransactions = async (userId, userFirstLoginDate, limit = 50, offset = 0) => {
  if (Capacitor.getPlatform() !== "android") {
    return { transactions: [] };
  }
  try {
...
var toDashboardTransaction = (entry) => ({
  id: entry.id,
  smsHash: entry.smsHash || void 0,
  amount: entry.amount,
  ...
  idempotencyKey: entry.idempotencyKey || void 0,
  updatedAt: entry.updatedAt || void 0
});
var getSalaryAmount = (salary) => {
  if (!salary) return 0;
  return Number(salary.amount ?? salary.monthly_salary ?? 0) || 0;
};

// src/integrations/sqliteService.ts
init_sqlite();
init_dist();
var SQLITE_TABLE_NAME = /^[A-Za-z_][A-Za-z0-9_]*$/;
var seedLocalCacheRow = async (tableName, row, syncStatus = "completed") => {
  const db2 = await ensureDbReady();
  if (!db2 || !row || !SQLITE_TABLE_NAME.test(tableName)) return false;
  try {
    const tableInfo = await db2.query(`PRAGMA table_info(${tableName})`);
    const validCols = (tableInfo.values || []).map((v) => v.name);
    ...
    if (validCols.includes("sync_status") && seedRow.sync_status === void 0) {
      seedRow.sync_status = syncStatus;
    }
    if (validCols.includes("is_deleted") && seedRow.is_deleted === void 0) {
      seedRow.is_deleted = 0;
    }
    const rowCols = Object.keys(seedRow).filter((col) => validCols.includes(col) && seedRow[col] !== void 0);
    ...
    if (rowCols.includes("id")) {
      const updates = rowCols.filter((col) => col !== "id").map((col) => `${col} = excluded.${col}`).join(", ");
      await db2.run(
      ...
      await db2.run(`INSERT OR IGNORE INTO ${tableName} (${colsString}) VALUES (${placeholders})`, values);
    }
    return true;
  } catch (err) {
...
var fetchLocalOrCloud = async (tableName, filterValue, extraCondition = "", orderBy = "created_at DESC", filterColumn = "user_id", includeDeleted = false, forceCloud = false) => {
  const isAndroid = Capacitor.getPlatform() === "android";
  if (isAndroid) {
    const db2 = await ensureDbReady();
    if (db2) {
      console.log(`\u{1F50D} [SQLITE_QUERY_START] Table: ${tableName}, Filter: ${filterColumn || "NONE"}=${filterValue || "NONE"}, ForceCloud: ${forceCloud}`);
      ...
      try {
        const res = await db2.query(query, params);
        const localRows = res.values || [];
        console.log(`\u{1F4CA} [SQLITE_QUERY_RESULT] Table: ${tableName}, Count: ${localRows.length}`);
        const countRes = await db2.query(countQuery, countParams);
        const totalCount = countRes.values?.[0]?.count || 0;
        if (!forceCloud && (localRows.length > 0 || totalCount > 0)) {
          console.log(`\u{1F4A1} [SQLITE_HYDRATED] Returning local cache (Total historical rows: ${totalCount})`);
          return localRows;
        }
        console.log(`\u{1F4A1} [SQLITE_FETCH_REQUIRED] ${forceCloud ? "Force Refresh" : "Local Empty"} - Fetching from cloud for ${tableName}`);
      } catch (err) {
        console.error(`\u274C [SQLITE_QUERY_FAIL] Table: ${tableName}`, err);
      }
    }
  }
  console.log(`\u{1F310} [CLOUD_FETCH_START] Table: ${tableName}, Filter: ${filterColumn || "NONE"}=${filterValue || "NONE"}`);
  ...
  const { data, error } = await cloudQuery.order(orderBy.split(" ")[0], { ascending: !orderBy.includes("DESC") }).limit(500);
  if (error) {
  ...
  if (isAndroid && cloudRows.length > 0) {
    const db2 = await ensureDbReady();
    if (db2) {
      console.log(`\u{1F331} [SQLite_SEED] Seeding ${cloudRows.length} records into ${tableName}`);
      try {
...
// src/hooks/useDashboardData.ts
var toLocalStart = (date) => startOfDay(date);
var toLocalEnd = (date) => endOfDay(date);
var useDashboardData = (user, canReadSms, ledgerWindow, currentMonthYear, dateFilter, isSQLiteReady = true) => {
  const queryClient = useQueryClient();
  const now = (0, import_react.useMemo)(() => /* @__PURE__ */ new Date(), []);
  const [isLoadingNativeTransactions, setIsLoadingNativeTransactions] = (0, import_react.useState)(false);
  const isAndroid = Capacitor.getPlatform() === "android";
  const feederInProgressRef = (0, import_react.useRef)(false);
  ...
  const { data: ledgerTransactions = [], isLoading: loadingExpenses } = useQuery({
    queryKey: ["ledger-transactions", user?.id, ledgerWindow.start.toISOString(), ledgerWindow.end.toISOString()],
    queryFn: async () => {
      if (!user?.id) return [];
      if (isAndroid && !isSQLiteReady) {
        console.warn("\u{1F9EA} [DASHBOARD_DATA] SQLite not ready. Aborting ledger fetch.");
        return [];
      }
      console.log("\u{1F9EA} [DASHBOARD_DATA] Fetching ledger. isSQLiteReady:", isSQLiteReady);
      return await fetchUnifiedLedger(user.id, ledgerWindow);
    },
    // 🚀 FIX: Strictly disable query until SQLite is ready to prevent caching empty arrays
    enabled: !!user?.id && (isAndroid ? isSQLiteReady : true),
    staleTime: 3e4
  });
  const loadNativeTransactions = (0, import_react.useCallback)(async () => {
    ...
    try {
      ...
      let currentOffset = 0;
      let hasMore = true;
      const { saveLocalTransaction: saveLocalTransaction2 } = await Promise.resolve().then(() => (init_sqlite(), sqlite_exports));
      console.log(`\u{1F680} [BRIDGE_FEEDER_START] PageSize: ${PAGE_SIZE}`);
        ...
        for (const tx of transactions) {
          if (isValidSmsTransaction(tx)) {
            ...
            await saveLocalTransaction2({
              ...
              // Native records are considered local truth
            }, true);
          }
        }
  ...
  (0, import_react.useEffect)(() => {
    ...
    let isMounted = true;
    const handleNewLocal = (event) => {
      console.log(`\u{1F504} [DASHBOARD_DATA] ${event.type} event detected, invalidating ledger for instant UI update.`);
      queryClient.invalidateQueries({ queryKey: ["ledger-transactions", user?.id] });
    };
    const handleNewSms = async (data) => {
      console.log(`[FORENSIC_TRACE] INSIDE JS addListener ('newTransaction' received)`, data);
      console.log(`\u{1F504} [DASHBOARD_DATA] newTransaction (SMS Bridge) detected. Triggering feeder pull.`);
      if (isMounted) {
        console.log(`[FORENSIC_TRACE] BEFORE loadNativeTransactions`);
        await loadNativeTransactions();
        console.log(`[FORENSIC_TRACE] AFTER loadNativeTransactions`);
        console.log(`[FORENSIC_TRACE] BEFORE invalidateQueries('ledger-transactions')`);
        queryClient.invalidateQueries({ queryKey: ["ledger-transactions", user?.id] });
        console.log(`[FORENSIC_TRACE] AFTER invalidateQueries('ledger-transactions')`);
      }
    };
  ...
  const salaryTransaction = (0, import_react.useMemo)(() => {
    const salaryAmount = getSalaryAmount(salaryData);
    if (!salaryAmount) return null;
    return toDashboardTransaction(
      toUnifiedSalaryEntry(
        salaryAmount,
        salaryData?.created_at ?? `${currentMonthYear}-01T00:00:00.000Z`
      )
    );
  }, [currentMonthYear, salaryData]);
  const allUnifiedTransactions = (0, import_react.useMemo)(() => {
    const salaryEntries = salaryTransaction ? [toUnifiedSalaryEntry(
      getSalaryAmount(salaryData),
      salaryData?.created_at ?? `${currentMonthYear}-01T00:00:00.000Z`
    )] : [];
    return mergeUnifiedLedgerEntries([
      ...salaryEntries,
      ...ledgerTransactions
    ]).map(toDashboardTransaction);
  }, [ledgerTransactions, salaryTransaction, salaryData, currentMonthYear]);
  const currentMonthExpenses = (0, import_react.useMemo)(() => {
  ...
  const lastMonthExpenses = (0, import_react.useMemo)(() => {
    const transactions = allUnifiedTransactions || [];
    return transactions.filter((tx) => {
      if (!tx?.date || !isValidDate(tx.date)) return false;
      const d = new Date(tx.date);
      return tx.type !== "income" && isLastMonth(d, now);
    });
  }, [allUnifiedTransactions, now]);
  const filteredViewData = (0, import_react.useMemo)(() => {
    const transactions = allUnifiedTransactions || [];
